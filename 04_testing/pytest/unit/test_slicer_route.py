...
def test_slicer_status_reports_readiness_without_slicing(
    isolated_app: tuple, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    import hermes3d.api.routes.slicer as slicer_route

    client, _ = isolated_app
    fake_slicer = tmp_path / "prusa-slicer-console.exe"
    fake_slicer.write_text("fake binary for status probe only", encoding="utf-8")
    monkeypatch.setattr(slicer_route, "_detect_slicer_binary", lambda: fake_slicer)

    resp = client.get("/api/slicer/status")

    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["accepted"] is True
    assert body["status"] == "proof_required"
    assert body["execution_ready"] is True
    assert body["proof_ready"] is False
    assert body["slicer_binary"] == str(fake_slicer)
    assert body["endpoints"]["submit"] == "/api/slice"
    assert body["freeze"]["no_printer_writes"] is True
    assert body["freeze"]["no_dispatch"] is True
    assert "No valid latest slice_completed" in body["blockers"][0]


def test_slicer_status_reports_latest_gcode_and_proof(
    isolated_app: tuple, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    import hermes3d.api.routes.slicer as slicer_route
    from hermes3d.api.routes._common import as_json, execute

    client, _ = isolated_app
    fake_slicer = tmp_path / "orca-slicer.exe"
    fake_slicer.write_text("fake binary for status probe only", encoding="utf-8")
    monkeypatch.setattr(slicer_route, "_detect_slicer_binary", lambda: fake_slicer)

    job_id = "slice-status-job-1"
    gcode = tmp_path / "part.gcode"
    gcode.write_text("; generated by PrusaSlicer\nG1 X0 Y0 E1\n", encoding="utf-8")
    proof = tmp_path / "proof.json"
    proof.write_text('{"schema":"hermes3d.slicer.proof.v1"}', encoding="utf-8")
    gcode_sha = hashlib.sha256(gcode.read_bytes()).hexdigest()
    proof_sha = hashlib.sha256(proof.read_bytes()).hexdigest()

    execute(
        """
        INSERT INTO jobs (id, name, job_type, status, dry_run)
        VALUES (?, 'Slice status proof', 'slice', 'completed', 1)
        """,
        (job_id,),
    )
    execute(
        ...
        (
            ...
            str(gcode),
            gcode.stat().st_size,
            as_json({"sha256": gcode_sha, "layer_count": 1, "motion_lines": 1}),
        ),
    )
    execute(
        """
        ...
        (
            ...
            str(proof),
            proof.stat().st_size,
            as_json({"sha256": proof_sha, "gcode_artifact_id": "gcode-artifact-1"}),
        ),
    )
    execute(
        "INSERT INTO proof_events (id, event_type, source_agent, payload) VALUES (?, 'slice_completed', 'slicer-executor', ?)",
        (
            "slice-proof-event-1",
            as_json(
                {
    ...
    execute(
        """
        INSERT INTO jobs (id, name, job_type, status, dry_run, created_at, updated_at)
        VALUES (
            'newer-cancelled-slice',
            'Later cancelled slice job',
            'slice',
            'cancelled',
            1,
    ...
    assert body["execution_ready"] is True
    assert body["proof_ready"] is True
    assert body["latest_job"]["id"] == "newer-cancelled-slice"
    assert body["latest_success_job"]["id"] == job_id
    assert body["latest_gcode"]["id"] == "gcode-artifact-1"
    assert body["latest_gcode"]["valid"] is True
    assert body["latest_gcode"]["sha256_checked"] is True
    assert body["latest_gcode"]["sha256_match"] is True
    assert body["latest_proof"]["id"] == "proof-artifact-1"
    assert body["latest_proof_event"]["id"] == "slice-proof-event-1"
    assert body["latest_proof_event"]["event_type"] == "slice_completed"
...
def test_slice_routes_are_registered() -> None:
    ...
    assert "/api/slicer/status" in paths
