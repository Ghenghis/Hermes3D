...
def test_skill_store_add_lookup_persist(tmp_path: Path):
    ...
    found = store.lookup(
        kind=SkillKind.PARAMETER_OVERRIDE, printer_id="tronxy_d01_pro", material="ASA"
    )
    assert len(found) == 1
...
def test_skill_store_specificity_ordering(tmp_path: Path):
    ...
    matches = store.lookup(
        kind=SkillKind.PARAMETER_OVERRIDE, printer_id="flsun_t1_a", material="PLA"
    )
...
def test_skill_store_reinforce_caps_at_one(tmp_path: Path):
    ...
    store = SkillStore(tmp_path / "sk.json")
    s = store.add(
        skill_kind=SkillKind.USER_PREFERENCE, name="x", scope=SkillScope(), body={}, confidence=0.95
    )
    for _ in range(10):
...
def test_skill_scope_specificity_count():
    ...
    assert (
        SkillScope(
            printer_id="x", material="PLA", quality_level="fine", hour_of_day=14
        ).specificity()
        == 4
    )
...
def test_failure_predict_skill_dominates(tmp_path: Path):
    ...
    f = predict_failure(printer_id="creality_cr10s", material="PETG", skills=sk)
