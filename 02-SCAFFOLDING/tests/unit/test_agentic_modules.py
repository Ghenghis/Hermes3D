...
def test_print_history_append_and_iter(tmp_path: Path):
    ...
    h.append(job_id="a", printer_id="flsun_t1_a", material="PLA",
             started_unix=1000.0, ended_unix=4600.0, success=True,
             filament_used_g=42)
    h.append(job_id="b", printer_id="prusa_mk3s", material="PETG",
             started_unix=5000.0, ended_unix=11000.0, success=False,
...
def test_print_history_aggregate_metrics(tmp_path: Path):
    ...
    for i in range(5):
        h.append(job_id=f"j{i}", printer_id="flsun_t1_a", material="PLA",
                 started_unix=1000.0 + i, ended_unix=4600.0 + i,
                 success=(i != 2), filament_used_g=20)
...
def test_cost_estimate_pla_short_print():
    ...
    e = estimate_cost(printer_id="flsun_t1_a", material="PLA",
                       filament_g=110, duration_hours=6.0)
...
def test_cost_estimate_unknown_material_uses_fallback():
    from hermes3d.core.farm.cost_estimator import estimate_cost
    e = estimate_cost(printer_id="prusa_mk3s", material="UNOBTAINIUM",
                       filament_g=100, duration_hours=2)
...
def test_cost_estimate_negative_inputs_raise():
    ...
    with pytest.raises(ValueError):
        estimate_cost(printer_id="prusa_mk3s", material="PLA",
                       filament_g=-1, duration_hours=1)
    with pytest.raises(ValueError):
        estimate_cost(printer_id="prusa_mk3s", material="PLA",
                       filament_g=10, duration_hours=-1)
...
def test_equivalence_least_busy_prefers_ready():
    from hermes3d.core.agents.equivalence import (
        DEFAULT_GROUPS, least_busy_member,
    )
...
def test_scheduler_short_print_at_3pm_allowed():
    ...
    d = schedule_window(duration_minutes=120, policy=SchedulerPolicy(),
                          now=now)
    assert d.allowed
...
def test_scheduler_quiet_hours_blocks_start():
    ...
    d = schedule_window(duration_minutes=240, policy=SchedulerPolicy(),
                          now=now)
...
def test_queue_enqueue_dispatch_slice_succeed_flow(tmp_path: Path):
    ...
    q.transition_job(j.job_id, JobState.DISPATCHED,
                       target_printer_id="flsun_t1_a")
    q.transition_job(j.job_id, JobState.VALIDATED)
    q.transition_job(j.job_id, JobState.SLICED,
                       sliced_gcode_path="/x.gcode")
...
def test_queue_persists_across_reload(tmp_path: Path):
    ...
    q1.transition_job(j.job_id, JobState.DISPATCHED,
                        target_printer_id="prusa_mk3s")
    # Reload from disk
    q2 = JobQueue(qpath)
    j2 = q2.get(j.job_id)
...
def test_spool_tracker_add_load_consume(tmp_path: Path):
    ...
    s = st.add(material="PLA", color="black", color_hex="#000",
                vendor="Polymaker", initial_grams=1000)
...
def test_spool_tracker_loading_unloads_previous(tmp_path: Path):
    from hermes3d.core.farm.spool_tracker import SpoolTracker
    st = SpoolTracker(tmp_path / "sp.json")
    s1 = st.add(material="PLA", color="A", color_hex="#A",
                 vendor="V", initial_grams=500)
    s2 = st.add(material="PLA", color="B", color_hex="#B",
                 vendor="V", initial_grams=500)
...
def test_spool_tracker_consume_negative_raises(tmp_path: Path):
    from hermes3d.core.farm.spool_tracker import SpoolTracker
    st = SpoolTracker(tmp_path / "sp.json")
    s = st.add(material="PLA", color="A", color_hex="#A",
                vendor="V", initial_grams=500)
    with pytest.raises(ValueError):
...
def test_notifier_payload_shapes():
    ...
    from hermes3d.core.notifications.notifier import (
        discord_payload, slack_payload, NotificationEvent, NotificationLevel,
    )
    e = NotificationEvent(title="Print done", message="OK",
                            level=NotificationLevel.SUCCESS,
                            printer_id="flsun_t1_a", job_id="abc12345")
    d = discord_payload(e)
...
def test_preflight_passes_clean_scenario():
    ...
    spool = Spool(
        spool_id="s", material="PLA", color="black",
        color_hex="#000", vendor="V", diameter_mm=1.75,
        initial_grams=1000, remaining_grams=900,
        loaded_on_printer="flsun_t1_a",
    )
    cost = estimate_cost(printer_id="flsun_t1_a", material="PLA",
                          filament_g=110, duration_hours=6.0)
    schd = schedule_window(duration_minutes=360, policy=SchedulerPolicy(),
                            now=dt.datetime(2026, 4, 29, 10, 0))
    r = run_preflight(
        printer_id="flsun_t1_a",
        truth_gate_report=FakeTG(),
        live_state={"reachable": True, "klippy_state": "ready"},
        spool=spool, required_grams=110, required_material="PLA",
        schedule_decision=schd, cost_estimate=cost, budget_usd=50.0,
...
def test_preflight_fails_when_spool_too_low():
    ...
    spool = Spool(
        spool_id="s", material="PLA", color="black",
        color_hex="#000", vendor="V", diameter_mm=1.75,
        initial_grams=1000, remaining_grams=50,
    )
    r = run_preflight(
        printer_id="flsun_t1_a",
        spool=spool, required_grams=200, required_material="PLA",
    )
    assert not r.passed
...
def test_preflight_fails_on_wrong_material():
    ...
    from hermes3d.core.farm.spool_tracker import Spool
    spool = Spool(
        spool_id="s", material="PETG", color="x", color_hex="#x",
        vendor="V", diameter_mm=1.75,
        initial_grams=1000, remaining_grams=900,
    )
    r = run_preflight(
        printer_id="flsun_t1_a",
        spool=spool, required_grams=100, required_material="PLA",
    )
    assert not r.passed
...
def test_backup_restore_roundtrip(tmp_path: Path):
    ...
    st.add(material="PLA", color="x", color_hex="#x",
           vendor="V", initial_grams=500)
...
def test_backup_refuses_overwrite_by_default(tmp_path: Path):
    ...
    state = tmp_path / "v"
    state.mkdir()
    JobQueue(state / "queue.json").enqueue(
        mesh_path="/x", mesh_sha256="a", material="PLA")
...
def test_api_dispatch_returns_selection(api_client):
    r = api_client.post("/dispatch", json={
...
def test_api_queue_lifecycle(api_client):
    r = api_client.post("/queue/jobs", json={
        "mesh_path": "/tmp/x.stl",
        "mesh_sha256": "abc",
        "material": "PLA",
    })
...
def test_api_spool_lifecycle(api_client):
    r = api_client.post("/spools", json={
        "material": "PLA", "color": "black", "color_hex": "#000",
        "vendor": "Polymaker", "initial_grams": 1000,
    })
    assert r.status_code == 200
    spool_id = r.json()["spool_id"]
    r = api_client.post(f"/spools/{spool_id}/load",
                        json={"printer_id": "flsun_t1_a"})
    assert r.status_code == 200
    r = api_client.post(f"/spools/{spool_id}/consume",
                        json={"grams": 50.0})
    assert r.status_code == 200
