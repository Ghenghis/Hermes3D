#!/usr/bin/env python3
"""Assemble the Phase 3.1 read-only fleet proof bundle.

The bundle is intentionally evidence-only: it snapshots docs, a SQLite ledger,
fixture replay data, UI build hashes, and the Playwright JSON report. It also
verifies that the sidecar manifest exactly matches zip contents.
"""
...
OUTPUT_DIR = REPO_ROOT / "06_release" / "phase3.1-bundle"
DIST_DIR = REPO_ROOT / "03_implementation" / "ui" / "dist"

REQUIRED_DOCS = {
    "docs/PHASE3_PLAN.md": REPO_ROOT / "00_overview" / "PHASE3_PLAN.md",
    "docs/ADR-009-orchestration-skeleton.md": REPO_ROOT
    / "02_architecture"
    / "adr"
    / "ADR-009-orchestration-skeleton.md",
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"{run_id}-") as tmp:
        ...
        _write_ledger_snapshot(ledger_snapshot)
        entries["evidence/ledger_snapshot.sqlite3"] = ledger_snapshot

        fixture_replay = staging / "fixture_replay_log.json"
        _write_fixture_replay_log(fixture_replay)
        entries["evidence/fixture_replay_log.json"] = fixture_replay
...
def _write_ledger_snapshot(path: Path) -> None:
    ...
    ledger = OrchestrationLedger(path)
    ledger.append(
        LedgerEvent(
            ts_utc="2026-05-01T00:00:00Z",
            run_id="phase3.1-proof",
            agent_id="proof-fixture-agent",
            tool="printer.poll",
            inputs_sha=hashlib.sha256(b"fixture-inputs").hexdigest(),
            outputs_sha=hashlib.sha256(b"fixture-outputs").hexdigest(),
            verdict="pass",
            message="fixture poll snapshot recorded",
        )
    )


def _write_fixture_replay_log(path: Path) -> None:
    sys.path.insert(0, str(REPO_ROOT / "03_implementation" / "src"))
    from hermes3d.orchestration.bridge import fixture_printer_snapshot

    printers = fixture_printer_snapshot()
    payload = {
        "route": "GET /api/printers",
        "total": len(printers),
        "live": sum(1 for printer in printers if printer["data_source"] == "live"),
        "mock": sum(1 for printer in printers if printer["data_source"] == "mock"),
        "printers": printers,
    }
...
def _build_manifest(run_id: str, zip_path: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase3.1-bundle-1.0.0",
        "run_id": run_id,
