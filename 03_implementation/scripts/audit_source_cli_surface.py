...
def main() -> int:
    ...
    candidates = [record["module_id"] for record in records if record["cli_surface_status"].endswith("_needs_verifier")]
    audit = {
        ...
        "summary": {
            ...
            "no_local_cli_signal": sum(1 for record in records if record["cli_surface_status"] == "no_local_cli_signal"),
        },
        "records": records,
        "next_actions": [
...
def classify_cli_surface(module: dict[str, Any], runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    if agent_tier == "verified_agent_cli":
        ...
        next_action = "Keep as launcher metadata only until a safe CLI/API/desktop-bridge smoke is proven."
        agent_enabled = False
    elif any(signal["kind"] in {"package_bin", "python_console_script", "root_executable"} for signal in source_signals):
...
def agent_execution_tier(module: dict[str, Any], runtime: dict[str, Any]) -> str:
    ...
    if runtime_status == "ready" and (proof_gate == "desktop-launcher-metadata-v1" or (kind == "desktop_app" and not executed)):
...
def package_json_signals(path: Path) -> list[dict[str, str]]:
    ...
    if isinstance(bin_value, str):
        signals.append({"kind": "package_bin", "source": "package.json", "name": str(pkg.get("name") or "bin"), "command": bin_value})
    elif isinstance(bin_value, dict):
        for name, command in list(bin_value.items())[:8]:
            signals.append({"kind": "package_bin", "source": "package.json", "name": str(name), "command": str(command)})
    scripts = pkg.get("scripts")
    if isinstance(scripts, dict):
        for name, command in scripts.items():
            lname = str(name).lower()
            if lname in {"start", "serve", "dev", "cli"} or "cli" in lname or "server" in lname:
                signals.append({"kind": "package_script", "source": "package.json", "name": str(name), "command": str(command)[:220]})
    return signals[:12]
    ...
    if isinstance(project, dict):
        ...
        if isinstance(scripts, dict):
            for name, target in list(scripts.items())[:12]:
                signals.append({"kind": "python_console_script", "source": "pyproject.toml", "name": str(name), "command": str(target)})
...
def setup_py_signals(path: Path) -> list[dict[str, str]]:
    ...
    return [
        {"kind": "python_console_script", "source": "setup.py", "name": item.split("=", 1)[0].strip(), "command": item.strip()[:220]}
        for item in matches[:12]
    ] or [{"kind": "python_console_script", "source": "setup.py", "name": "console_scripts", "command": "console_scripts declared"}]
...
def makefile_signals(path: Path) -> list[dict[str, str]]:
    ...
    for line in lines[:300]:
        ...
        if match and match.group(1).lower() in {"run", "serve", "start", "cli", "dev", "server"}:
            signals.append({"kind": "make_target", "source": path.name, "name": match.group(1), "command": f"make {match.group(1)}"})
    return signals[:8]
...
def root_executable_signals(path: Path) -> list[dict[str, str]]:
    ...
    for child in sorted(children, key=lambda item: item.name.lower()):
        if child.is_file() and child.suffix.lower() in {".exe", ".cmd", ".bat", ".ps1"}:
            signals.append({"kind": "root_executable", "source": "root", "name": child.name, "command": str(child)})
    return signals[:8]
...
def readme_signals(path: Path) -> list[dict[str, str]]:
    ...
    for line in lines[:500]:
        ...
        if cleaned and README_SIGNAL_RE.search(cleaned):
            signals.append({"kind": "readme_cli_signal", "source": readme.name, "name": "docs", "command": cleaned[:220]})
        if len(signals) >= 8:
            break
    return signals
