...
export function KpiCard({ label, value, delta, chart, icon }: KpiCardProps) {
  return (
    <div className="bg-surface border border-border rounded-card px-3 py-2.5 flex items-center gap-2 min-w-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-fg text-[26px] font-bold leading-none mt-0.5 tabular-nums">{value}</div>
        {delta && (
      ...
      {chart && <div className="shrink-0 h-10 w-24">{chart}</div>}
    </div>
  );
}
