...
> **STATUS UPDATE 2026-05-10 (W5-8b doc-fix):** 12 file paths were corrected to match the on-disk layout (`03_implementation/src/hermes3d/services/...`, `03_implementation/src/hermes3d/db/...`, actual UI tab names under `03_implementation/ui/src/tabs/` and `components/`, actual integration-test names under `04_testing/pytest/{unit,integration}/`). BLK-011 is no longer "upstream-blocked": v0.13 was promoted to production default by PR #160 (merged 2026-05-10T00:19Z). The "Upstream-Blocked" row in Part 5 is superseded by `E2E_BLOCKER_REGISTRY_2026-05-09.md` (BLK-011 status: `superseded-by-PR-#160`).

---

## Executive Summary

**447 tracked files inventoried** across `03_implementation/src/hermes3d/`, `03_implementation/ui/src/`, `04_testing/pytest/`, `04_testing/playwright/`, and `.github/workflows/`.[^447]

[^447]: Snapshot from `git ls-files` at audit time on branch `feat/hermes3d-7-complete-gui-repo-wiring`. Re-run `git ls-files` for an exact current count; the number drifts as Wave 5 PRs land.
...
| **code_history** | 1 | 1 active | 100% (1/1) | 100% (1/1) | BLK-003, BLK-009 (partial) | #141, #143 |
...
| 3 | `03_implementation/src/hermes3d/services/code_history.py` | code_history | recovery | active | 100% (8+5 tests) | 90% (TOCTOU, MCP tool partial) | BLK-003, BLK-004, BLK-005, BLK-006, BLK-009 | BLK-009 escalated; BLK-004/005/006 closed #137 | Migrate `_call_mcp_tool` to HTTP transport (downstream of BLK-009) |
| 4 | `04_testing/playwright/specs/dashboard-advanced.spec.ts` *(spec ready in branch but not yet committed)* | tests.e2e | ui.shell | active | n/a | 60% (visual baseline needed) | BLK-015 | Spec ready; no PNG baseline for CI | Generate CI baseline image from running app |
| 5 | `03_implementation/ui/src/tabs/Printers.tsx` | ui.tab | ui.tab | active | 40% (4/10 scenarios) | 10% (1 Playwright drill) | BLK-017 (re-sweep active UI) | Tab verified no-fake in sweep; drill coverage sparse | Add Playwright drill for live adapter interaction |
...
| 8 | `03_implementation/src/hermes3d/db/load_modules.py` | db.modules | db.modules | active | 92% (11/12 tests) | 33% (4 manual) | BLK-014 (schema), Bonus 12 #9/#10 | #147 closed IndexError + FD leak; schema fields missing | Backfill 60-app rows with (method, proof, env, rollback) |
...
| 14 | `03_implementation/ui/src/components/health/StatusPill.tsx` | ui.tab | ui.tab | active | 35% (3/8 scenarios) | 20% (1 Playwright, live data sparse) | BLK-017 (active UI no-fake) | Tab verified no-fake; drill coverage needs boost | Add Playwright drill for health-status live wire |
| 15 | `04_testing/pytest/unit/test_recovery_controller_cross_version.py` | tests.unit | recovery | active | 80% (16/20 scenarios) | 70% (14 with rollback proof) | BLK-012 RC v2 partial | RC v2 commits 1+2 tested; commit 3+ not yet | Add tests for RC v2 commit 3 (MiniMax fix) |
| 16 | `03_implementation/src/hermes3d/services/recovery_controller.py` | recovery | recovery | active | 50% (scaffold only; #149) | 0% (commits 1–2 code-complete, e2e pending) | BLK-012 RC v2 partial | Commits 1+2 landed #149; commits 3–5 paused per user | Merge commits 1+2 tests; await user authorization for 3–5 |
...
| 20 | `03_implementation/ui/src/components/agents/AgentCommandCenter.tsx` | ui.tab | ui.tab | active | 45% (4/8 scenarios) | 25% (2 Playwright, command response flow partial) | BLK-017 (no-fake sweep) | Tab verified no-fake; agent response flow drill sparse | Add Playwright drill for agent dispatch + response stream |
...
| 22 | `03_implementation/src/hermes3d/db/init.py` *(migrations dir not yet created; init.py is the current entrypoint)* | db.modules | db.modules | active | 100% (4 tests) | 25% (1 manual) | none | DB migration scaffolding ready | Promote `init.py` to `migrations/` package and add integration test for backward-compat migration path |
| 23 | `04_testing/pytest/unit/adapters/test_moonraker_readonly_contract.py` *(live integration spec pending; readonly contract suite shipped)* | tests.unit | tests.e2e | active | 60% (12/20 scenarios) | 40% (6 with live shim, 8 mocked) | BLK-016 (safety drill) | Moonraker readonly contract test ready; safety gate flow incomplete | Extend with safety gate e2e drill (build-plate clear) |
...
| 28 | `04_testing/playwright/specs/error-recovery.spec.ts` *(spec drafted; not yet committed)* | tests.e2e | tests.e2e | active | n/a | 70% (14/20 error scenarios) | BLK-016 (safety drill) | Error recovery e2e ready; printer safety flow sparse | Extend with printer safety error scenario drill |
| 29 | `03_implementation/src/hermes3d/api/routes/autonomous.py` | agents.providers | agents.providers | active | 85% (17/20 tests) | 30% (6 manual, autonomy proof sparse) | none | Autonomy routes ready; autonomy proof missing | Add Playwright drill for full autonomy mode flow |
...
## Part 3: Dead or Unknown Files

> **Section status (W5-8b doc-fix):** the 3 candidates listed in earlier revisions of this audit had `(if exists)` qualifiers and were not confirmed via `git ls-files`. Speculative file claims do not belong in a coverage audit, so they have been removed pending a confirmed sweep.

**Outcome:** 0 dead files confirmed at this audit pass. A separate `git ls-files --deleted` + TODO/FIXME sweep is deferred to the next audit phase. Files that survive that sweep will be listed here with their commit SHA of last touch and a `git log --follow` trail.

**Note:** A previous draft of this section spot-checked deprecated entries; per W5-8 audit P2 finding, those entries were removed because they were unverifiable by `test -f`.
...
**File:** `04_testing/pytest/unit/test_recovery_controller_cross_version.py` (extend; line ~180)  
...
**File:** `03_implementation/src/hermes3d/api/routes/code_operator.py` (new routes added; line ~150)  
