...
class PrintSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        self._listeners: list[Callable[[SupervisorEvent, PrinterState, dict[str, Any]], None]] = []
    ...
    def on_event(self, fn: Callable[[SupervisorEvent, PrinterState, dict[str, Any]], None]) -> None:
        self._listeners.append(fn)

    def _emit(self, event: SupervisorEvent, ps: PrinterState, detail: dict[str, Any]) -> None:
        ps.last_event = event
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._loop, name="hermes3d-supervisor", daemon=True)
        self._thread.start()
        log.info(
            "supervisor started (%d printers, %.1fs interval)",
            len(self.states),
    ...
    def _poll_printer(self, pid: str, ps: PrinterState) -> None:
        try:
            ...
            return
        client = MoonrakerClient(profile.moonraker_url_default, timeout_s=4.0)
        live = client.printer_state()
        ps.last_seen_unix = time.time()
        if not live.get("reachable", False):
            if ps.klippy_state != "offline":
                self._emit(SupervisorEvent.PRINTER_OFFLINE, ps, {"error": live.get("error")})
            ps.klippy_state = "offline"
            return

        new_state = live.get("klippy_state", "unknown")
        new_filename = (
            live.get("print_state", {}).get("filename")
            if isinstance(live.get("print_state"), dict)
            else None
        )
        ...
        if not was_printing and is_printing:
            ...
            self._emit(SupervisorEvent.PRINT_STARTED, ps, {"filename": new_filename})
        ...
        if is_printing and self.policy.obico_enabled and self.policy.obico_base_url:
    ...
    def _handle_print_ended(self, pid: str, ps: PrinterState, new_state: str) -> None:
        ...
        loaded = next((s for s in self.spools.list(printer_id=pid)), None)
        if loaded:
        ...
        if success and self.policy.notify_on_complete:
            ...
            self._emit(SupervisorEvent.PRINT_COMPLETED, ps, {"record_id": record.record_id})
    ...
    def _poll_obico_for(self, pid: str, ps: PrinterState) -> None:
        ...
        if status.recommended_action == ObicoAction.PAUSE:
            self._emit(
                SupervisorEvent.OBICO_PAUSE_RECOMMENDED,
                ps,
            ...
            if self.policy.auto_pause_on_obico:
                ...
                try:
                    profile = get_profile(pid)
                    moonraker = MoonrakerClient(profile.moonraker_url_default, timeout_s=4.0)
                    moonraker._request("POST", "/printer/print/pause")
                    self._emit(
                        SupervisorEvent.OBICO_AUTO_PAUSED,
                        ps,
                        {"failure_probability": status.failure_probability},
                    )
                except Exception as exc:  # noqa: BLE001
            ...
            self._emit(
                SupervisorEvent.OBICO_HEADS_UP,
                ps,
                {"failure_probability": status.failure_probability},
            )
    ...
    def _maybe_learn_failure(self, printer_id: str, material: str) -> None:
        ...
        self._failure_observation_counts[key] = self._failure_observation_counts.get(key, 0) + 1
            ...
            if rate >= 0.30:
                ...
                if existing:
                    ...
                    self.skills.add(
                        ...
                        scope=SkillScope(printer_id=printer_id, material=material),
                        body={"observed_failure_rate": round(rate, 3), "observation_count": count},
