...
| Agents rail/chat | UI_DONE_RUNTIME_PARTIAL | Browser mic, voice-note upload, file attachments, quick context prompts, Azure voice playback for agent replies, real runtime/SSE when configured, a bounded Hermes Agent Playwright proof runner, and the OS operator action catalog surface exist. This does not mean Hermes Agents can co-develop Hermes3D yet. The co-developer runtime remains P0 until MiniMax and DeepSeek teams complete a real task through folder-index context, MCP locks, snapshots, patch proposal/apply, gates, review, branch, push, PR, rollback proof, and lock release. |
...
- The folder index from PR #86 is now required agent boot context. Before an agent coding task starts, MiniMax and DeepSeek must load the relevant handoff index files under `03_implementation/docs/handoffs/`, name the files used in the task record, and prove the chosen files match the folder ownership context.
...
1. Build the Hermes Agent Code Workbench in Agents UI so the user can submit a real coding task with title, objective, files, target branch, provider team, and required roles.
2. Add backend orchestration for one E2E coding job: folder-index context -> programming/provider readiness -> task claim -> file locks -> pre-snapshots -> MiniMax coding pass -> DeepSeek review pass -> patch proposal -> same-owner apply -> gates -> post-snapshots -> branch/stage/commit/push/PR -> release evidence.
3. Add OpenHands/OpenCode CLI runner detection and read-only preflight as optional worker backends using the same MiniMax/DeepSeek provider pool; keep run blocked until sandbox, locks, snapshots, and proof capture are configured.
4. Add the first real smoke task that Hermes Agents complete end-to-end against a low-risk Hermes3D file, then record proof ids and PR URL.
