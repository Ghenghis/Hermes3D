...
Updated: 2026-05-07
Owner: codex-master
Task: H3D-CODEX-SOURCE-OS-RUNNER-CONTRACTS
...
2. Runtime verifier registry: 32 registered probe definitions are exposed by `/api/modules/runtime/verifiers`; the current matrix produces 30 verifier-backed ready rows. The next step is adding DB-backed runner rows and verifiers by app family.
...
The current provider/review execution lane is Codex PR `#85`; the active follow-up is this Source OS runner-contract lane, which turns the 30 runner gaps into exact non-executable contracts until each app has a sandboxed verifier, proof gate, and rollback story.
