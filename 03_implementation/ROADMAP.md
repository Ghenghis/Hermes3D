...
Updated: 2026-05-07
Owner: codex-master
Task: H3D-CODEX-SOURCE-OS-PYTHON-RUNNERS
...
| Source OS | IN_PROGRESS | Source registry truth audit currently proves 60/60 source-backed app rows. Runtime Readiness currently shows 31 verifier-backed ready apps, 17 source-ready runner-not-registered rows, 10 runtime-repair rows, and 2 blocked rows with explicit `ready/source/setup/install/blocked` badges, Verify All, Setup Queue, selected-app Verify, selected-app Setup Plan write proof, and Backup/Check Update/Update/Rollback live backend routes. `/api/modules/runtime/runner-contracts` now exposes the canonical 60-row Hermes Agent execution matrix: 7 rows are `agent_executable=true`, 29 rows remain runner gaps/repair blockers, and every non-executable row carries required verifier family, safe actions, and blocked reason. The registered verifier set produces 7 agent-usable CLI probes, 3 desktop launcher metadata probes, 13 read-only source-inventory probes, 5 package/import probes including Blender MCP source-import plus the MeshLab `pymeshlab` bridge, 5 Python import repair probes for CadQuery/Open3D/build123d/numpy-stl/PyMesh, 3 read-only Moonraker/Klipper fleet probes, and 5 local/private print-farm HTTP health probes for FDM Monster, Fluidd, Mainsail, OctoFarm, and OctoPrint; those print-farm rows stay setup-required until `HERMES3D_SOURCE_*_URL` values in `G:/private/.env` return real non-mutating health/version proof. Loader launch-kind overrides reduce the vague `unknown` bucket to 0/60. The CLI-surface audit proves 24 local CLI/service hints still need verifier/runner work before Hermes Agents can execute them. |
...
- `/api/modules/runtime/gaps` groups the remaining 29 runner gaps/repair blockers by launch kind and section, with the next safe verifier family needed for each group.
...
- `/api/modules/runtime/setup-queue` is now also available as a read-only GET status for Plugins, Settings Environment, and Roadmap so the 29 runner/repair gaps are visible without creating proof spam; POST records a proof-backed plan.
...
The operator target is 60 useful source-backed apps. Current local truth is 60 proved modules, 31 verifier-backed ready rows, 7 agent-usable CLI probes, 24 CLI/service signals needing verifiers, 17 source-ready rows without safe runners, 10 registered verifier repair rows, and 2 blocked rows. The completion path is:
...
2. Runtime verifier registry: 43 registered probe definitions are exposed by `/api/modules/runtime/verifiers`; the current matrix produces 31 verifier-backed ready rows. The next step is converting the remaining family-specific repair/gap rows into dry-run worker, service-health, CLI help/version, or reference-parser runners.
...
Current CLI readiness proof: `03_implementation/proof/SOURCE_APP_CLI_AGENT_READINESS_AUDIT.json`, `/api/modules/runtime/agent-cli-readiness`, and `/api/modules/runtime/runner-contracts` classify all 60 rows. As of this pass, 7 apps are verified agent CLI and agent-executable (`hermes_agent`, `blender`, `openscad`, `curaengine`, `flsun_slicer`, `orcaslicer`, `prusaslicer`), 3 are launcher metadata only (`printrun`, `bambustudio`, `cura`), 5 are package/import ready (`model_context_protocol`, `blender_mcp_candidates`, `manifold`, `meshlab`, `trimesh`), 3 are service/API ready, 13 are read-only source/reference ready, and 29 remain runner gaps or repair blockers needing a real CLI/API/import/service smoke before Hermes Agents can execute them. Strec3D is verified source-reference-only from its README/CMake source inventory; it is not agent-executable and no local safe CLI is claimed. The Python/CAD repair queue is explicit: `cadquery`, `open3d`, `build123d`, `numpy_stl`, and `pymesh` have registered import verifiers but the backend runtime cannot import them yet. The print-farm service/web queue is explicit too: `fdm_monster`, `fluidd`, `mainsail`, `octofarm`, and `octoprint` have registered local/private HTTP health verifiers, but remain setup-required unless their configured `HERMES3D_SOURCE_*_URL` endpoints respond to non-mutating GET probes. Separate CLI-surface proof lives at `03_implementation/proof/SOURCE_APP_CLI_SURFACE_AUDIT.json`; it records 24 local CLI/service hints that are candidates only until a safe verifier and proof gate exist.
...
The current provider/review execution lane is Codex PR `#85`; the active follow-up is the Source OS runner-contract/runner-family lane. PR `#87` added the canonical execution matrix, PR `#88` added the Python/CAD verifier-family slice, PR `#89` corrected slicer truth by making Strec3D source-reference-only while leaving missing SuperSlicer/Slic3r CLIs blocked, and the current print-farm slice registers local/private health probes for FDM Monster, Fluidd, Mainsail, OctoFarm, and OctoPrint without marking any of them runnable until a real configured endpoint responds.
