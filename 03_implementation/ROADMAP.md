...
| Agents rail/chat | UI_DONE_RUNTIME_PARTIAL | Browser mic, voice-note upload, file attachments, quick context prompts, Azure voice playback for agent replies, real runtime/SSE when configured, a bounded Hermes Agent Playwright proof runner, the OS operator action catalog surface, and the Agent Code Workbench planning/review loop exist. This does not mean Hermes Agents can fully co-develop Hermes3D yet. The co-developer runtime remains P0 until reviewed provider artifacts can become same-owner patch apply, gates, branch, push, PR, rollback proof, and lock release in one end-to-end job. |
...
- Current live smoke proof for this workbench slice: folder-index and MCP lock readiness pass, but MiniMax execution is blocked by HTTP 401 from the configured private endpoint/key. The route now returns a redacted blocked reason and evidence instead of crashing or claiming a usable provider.
...
2. PARTIAL in the current workbench slice: add backend orchestration for one E2E coding job through folder-index context -> programming/provider readiness -> file locks -> pre-snapshots -> MiniMax coding pass -> DeepSeek review pass -> evidence/release. Remaining: reviewed patch proposal -> same-owner apply -> gates -> post-snapshots -> branch/stage/commit/push/PR -> release evidence.
